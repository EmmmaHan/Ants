    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is None:
                ...
                # BEGIN Problem 9
                # add insect with container == False to the place already occupied with bodygurad
                if not self.ant.container and insect.can_contain(self.ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        spaces = 0
        while location != hive:
            if location.bees and spaces>=self.min_range and spaces<=self.max_range:
    ...
    def action(self, colony):
        ...
        if self.digesting == 0:
            if random_or_none(location.bees):
                return self.eat_bee(random_or_none(location.bees))
        else:
            self.digesting -= 1
...
# END Problem 13
    ...
    def __init__(self):
        ...
        self.ant = []
        # END Problem 13
    def double_armor(self,ants):
        if not ants in self.ant and not isinstance(ants,QueenAnt):
            ants.damage = ants.damage *2
            self.ant.append(ants)
        if ants.container: 
            ant_hide = ants.ant
            if ant_hide and not ant_hide in self.ant: #if ant is not None and not in self.ant
                ant_hide.damage = ant_hide.damage *2
                self.ant.append(ant_hide)
    ...
    def action(self, colony):
        ...
        location = self.place
        if not self.queen:
            Insect.reduce_armor(self,self.armor) #reduces fake Queen armor to 0
            return 

        while location.exit != None:
            if location.ant != None:
                self.double_armor(location.ant)
...
def make_slow(action): #action(self,colony)
    ...
    def times(duration):
            def global_time():        
                ...
                if duration >0:
                    duration -=1
                    if colony.time %2 == 0:     #ant.action for even colony.time
                        return action(colony)
                else:
                     return action(colony)         
            return global_time        #called with global_time(colony)
    return times
    # END Problem EC
...
def make_stun(action):
    ...
    def times(duration):
        def global_time():
            nonlocal duration
            if duration >0:     #no action for duration >0
                duration -=1
                return 
            else:
                return action(colony)           
